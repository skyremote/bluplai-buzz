...
async fn authenticate_media_read(
    ...
    // Upstream now requires authentication for every media read. Bluplai's
    // room binding remains an additional authorization check for protected
    // objects, never an alternative to relay membership.
    let auth_event = extract_blossom_auth(headers)?;
...
pub async fn head_blob(
    ...
    let MediaReadAuth { tenant, .. } = media_auth;
    let cache_control = blob_cache_control();
